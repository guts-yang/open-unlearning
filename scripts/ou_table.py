...
用法：
    python scripts/ou_table.py                      # 写 results/ou_table3.md
    python scripts/ou_table.py --top 20 --tol 0.02
    python scripts/ou_table.py --runs results/ou_table3_runs.jsonl --out /tmp/t.md
"""
...
def main():
    ...
    out.append(f"- 数据来源：`{args.runs}`（{len(rows)} 条）")
    ...
    for method in sorted(by_method):
        ...
        out.append(head + "）\n")
        out.append("| 超参串 | Mem | Priv | Utility | Agg | HM(Mem,Utility) | 命中 | 与靶值偏差 |")
        out.append("|---|---|---|---|---|---|---|---|")
