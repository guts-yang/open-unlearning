...
def main() -> None:
    ...
    if selected is None:
        payload = {"skipped": True, "reason": "no FQ-feasible candidate selected"}
    ...
    validated = controller.run_validation(record)
    (output_dir / "validation_summary.json").write_text(
        ...
        json.dumps(
            {
                ...
                "reason": (
                    ...
                    "do not reuse the TOFU FQ gate. This agent shell has no CUDA."
                ),
            },
            indent=2,
        )
