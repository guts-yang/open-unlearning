...
#   # trainer.* 只传给 train.py，不传 eval.py（eval 配置没有 trainer）。
#   SKIP_TRAIN=1  # 只评已有 ckpt，写 results/tofu_<model>.md
...
# eval.yaml 没有 trainer；trainer.* 只留给训练
EXTRA_TRAIN_HYDRA=()
EXTRA_EVAL_HYDRA=()
...
if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
  ...
  echo "[info] SKIP_TRAIN=1，跳过训练，直接评 $ckpt"
  if [[ ! -f "$ckpt/config.json" && ! -f "$ckpt/model.safetensors" && ! -d "$ckpt" ]]; then
    echo "[error] ckpt 不存在：$ckpt"; exit 1
  fi
fi
